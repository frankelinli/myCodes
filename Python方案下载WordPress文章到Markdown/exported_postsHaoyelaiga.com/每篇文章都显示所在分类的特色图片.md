...
如果你想使用 **ACF** 和 **Astra 子主题**
来为每个分类添加特色图像，并在分类下的文章页面中显示当前分类的特色图像，这可以通过以下步骤实现。我们将利用 **ACF Pro** 的 **Term
Meta 功能** 来为分类添加自定义字段，同时结合 Astra 子主题的布局和钩子实现功能。
...
确保你的子主题有一个 `category.php` 文件。如果没有，可以从 Astra
主主题中复制一个模板文件到子主题中。然后，添加以下代码到适当的位置（比如分类标题的上方）：
